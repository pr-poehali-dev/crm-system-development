...
export default function CRMLayout({ activeModule, onModuleChange, children, rightPanel, rightPanelTitle, onCloseRightPanel }: Props) {
  ...
  return (
    <div className="flex h-screen bg-[#0f1117] text-white font-['Golos_Text']">
      {/* Sidebar */}
      <aside className="w-56 flex-shrink-0 bg-[#161b27] border-r border-[#252d3d] flex flex-col">
        {/* Logo */}
        <div className="h-14 flex items-center px-4 border-b border-[#252d3d]">
        ...
        {/* Office Switcher */}
        <div className="px-3 py-3 border-b border-[#252d3d] relative">
          <button
          ...
          {officeDropdown && (
            <div className="absolute left-3 right-3 top-full mt-1 z-50 bg-[#1e2637] border border-[#252d3d] rounded-lg shadow-xl py-1">
              ...
              <div className="border-t border-[#252d3d] mt-1 pt-1">
                <button
                  onClick={() => { onModuleChange('settings'); setOfficeDropdown(false); }}
      ...
      <div className="flex-1 flex flex-col min-w-0">
        {/* Header */}
        <header className="h-14 bg-[#161b27] border-b border-[#252d3d] flex items-center px-6 gap-4 flex-shrink-0">
          <div className="flex-1">
            <h1 className="text-base font-semibold text-white">
              {NAV_ITEMS.find((i) => i.id === activeModule)?.label || 'Главная'}
            </h1>
            <div className="text-xs text-[#4b5568]">{currentOffice?.name} • {new Date().toLocaleDateString('ru-RU', { day: 'numeric', month: 'long', year: 'numeric' })}</div>
          </div>
          <div className="flex items-center gap-2">
        ...
        <div className="flex-1 flex overflow-hidden">
          ...
          {/* Right Panel */}
          {rightPanel && (
            <div className="w-[420px] flex-shrink-0 bg-[#161b27] border-l border-[#252d3d] flex flex-col animate-slide-in-right">
              <div className="h-14 flex items-center px-5 border-b border-[#252d3d] flex-shrink-0">
                <span className="font-semibold text-sm text-white flex-1">{rightPanelTitle || 'Детали'}</span>
