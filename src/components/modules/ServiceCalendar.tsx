...
export default function ServiceCalendar({ onOpenPanel, onClosePanel }: Props) {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Calendar Grid */}
      <div className="bg-[#161b27] border border-[#252d3d] rounded-xl overflow-x-auto">
        <div className="min-w-[640px]">
          {/* Header row: Время + дни */}
          <div className="grid border-b border-[#252d3d]" style={{ gridTemplateColumns: '80px repeat(7, 1fr)' }}>
            ...
            {weekDays.map((day) => {
              ...
              const special = settings.specialDates?.find(sd => sd.date === ds);
              return (
                <div key={day.toISOString()} className={`px-2 py-3 text-center border-l border-[#252d3d] ${isWE ? 'bg-[#0f1117]' : ''}`}>
          ...
          {allSlotTimes.length === 0 ? (
            ...
            allSlotTimes.map((slotTime) => (
              <div key={slotTime} className="grid border-b border-[#252d3d] last:border-0" style={{ gridTemplateColumns: '80px repeat(7, 1fr)' }}>
                {/* Time label */}
                ...
                {weekDays.map((day) => {
                  ...
                  if (!slotDef) {
                    return (
                      <div key={day.toISOString()} className={`border-l border-[#252d3d] ${isWE ? 'bg-[#0f1117]' : 'bg-[#0d1018]'} min-h-[64px]`} />
                    );
                  }
                  ...
                  return (
                    <div key={day.toISOString()} className={`border-l border-[#252d3d] p-1.5 min-h-[64px] ${isWE ? 'bg-[#0f1117]' : 'hover:bg-[#1e2637]'} transition-colors`}>
                      {/* Brigades capacity bar */}
                      <div className="flex items-center justify-between mb-1">
                        <span className={`text-[10px] ${freeSlots > 0 ? 'text-[#10b981]' : 'text-[#ef4444]'}`}>
                      ...
                      {connsHere.map((conn) => {
                        ...
                        return (
                          <div
                            ...
                            className={`${st.bg} rounded p-1.5 mb-1 cursor-pointer hover:opacity-80 transition-opacity`}
                          >
                            <div className={`text-xs font-medium ${st.color} truncate leading-tight`}>{conn.subscriberName}</div>
                            {conn.subscriberAddress && (
                              <div className="text-[10px] text-[#4b5568] truncate">{conn.subscriberAddress}</div>
                            )}
                          </div>
                        );
                      })}
